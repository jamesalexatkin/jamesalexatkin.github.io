...
(function () {
    ...
    function updateDisplay() {
        ...
        clockEl.classList.toggle("pomodoro-clock--break", !isWorkSession);
        statusEl.classList.toggle("pomodoro-status--break", !isWorkSession);
        document.title =
            (!isRunning ? "⏸ " : "") +
    ...
    function tick() {
        if (secondsRemaining <= 0) {
            ...
            secondsRemaining = isWorkSession
                ? getWorkSeconds()
                : getBreakSeconds();
            // Notify user
            if (
